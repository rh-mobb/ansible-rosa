...
options:
    ...
    hosted_zone_id:
        description:
            - The unique zone identifier you want to modify / delete or "all" if there are many zones with the same domain name.
...
def find_zones(zone_in, private_zone):
    ...
    if (zone_id) and (zone_id != 'all'):
        try:
            ...
            module.fail_json_aws(e, msg=f"Could not get details about hosted zone {zone_id}")
        zones.append(result["HostedZone"])
    else:
        try:
            results = _list_zones()
        except (BotoCoreError, ClientError) as e:
    ...
    for z in matching_zones:
        ...
        if isinstance(vpc_details, dict) and len(record["vpcs"]) == 1:
            if vpc_details["VPC"]["VPCId"] == record["vpcs"][0]["id"]:
                current_vpcs.append(dict(
                    id = vpc_details["VPC"]["VPCId"],
                    region = vpc_details["VPC"]["VPCRegion"]
                ))
                matching = True
        else:
            ...
            if not matching:
                ...
                # check for vpcs to delete
                # module.fail_json_aws(None,f"current_vpcs: {current_vpcs}\n\nrecord: {record['vpcs']}")
                for vpc in current_vpcs:
                    if vpc not in record["vpcs"]:
                        try:
                            _ = client.disassociate_vpc_from_hosted_zone(
                            ...
                            module.fail_json_aws(e, msg=f"Could not remove vpc {vpc['region']}/{vpc['id']} association for hosted zone {zone_details['Id']}")
                        current_vpcs.remove(vpc)
                        record["msg"] += (f"Removed vpc {vpc['region']}/{vpc['id']} association to hosted zone {zone_details['Id']}.")
                        vpcs_changed = True
                # check for any new vpcs
                for vpc in record["vpcs"]:
                    # add vpc association
                    if vpc not in current_vpcs:
                        try:
                            _ = client.associate_vpc_with_hosted_zone(
                            HostedZoneId=result["HostedZone"]["Id"],
                            VPC={
                                'VPCId': vpc["id"],
                                'VPCRegion': vpc["region"]
                            ...
                            module.fail_json_aws(e, msg=f"Could not add vpc {vpc['region']}/{vpc['id']} association for hosted zone {zone_details['Id']}")
                        current_vpcs.append(vpc)
                        record["msg"] += (f"Added vpc {vpc['region']}/{vpc['id']} association to hosted zone {zone_details['Id']}.")
                        vpcs_changed = True
                if vpcs_changed:
                    record["vpcs"] = current_vpcs
                    matching = True
        ...
        if matching:
            ...
            if ("Comment" in zone_details["Config"]) and (zone_details["Config"]["Comment"] != record["comment"]):
                if (record["comment"]) and (not module.check_mode):
                    try:
            ...
            if not comment_changed:
                record['comment'] = zone_details["Config"]["Comment"] or ''
            if vpcs_changed or comment_changed:
                return True, record
            return False, record
